...
class ChaserInvaderEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if((d3-d1)>0 and (d3-d1)<self.thresh_distance):
            ...
            info = ["still working"]
            self.state = np.array([x,y, action_vel, action_phi, x_i,y_i, self.state[6], self.state[7], self.state[8],self.state[9],0,0.0]) #keep velocity of invader and base station constant
        return Step(observation=self.state, reward=reward, done=done,info=info)

    def reset(self):
        self.state = np.array([4.3324030411,2.6654027893,0.5721123503,0.748138597,28.3410776748,1.59902609,0.6269922575,-0.4922531189,10.898183446,10.9534020805,0,0])
