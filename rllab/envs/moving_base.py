...
class ChaserInvaderEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if((d3-d1)>0 and (d3-d1)<self.thresh_distance):
            ...
            x_i = self.state[4] - self.state[6]*self.state[7]
            y_i = self.state[5] - self.state[6]*self.state[7]
            ...
            #moving base station
            if(d1<=2):
                t_theta = ((self.state[1]-self.state[9])/(self.state[0]-self.state[8]))
                self.state[11] = math.tan(t_theta)
                x_b = self.state[8] + self.state[10]*math.cos(self.state[11])
                y_b = self.state[9] + self.state[10]*math.sin(self.state[11])
                ...
                if y_b > self.max_position:
                    y_b = 30.0
                #modify reward if the base station moves
                reward = reward - 2
                info = ["still working"]
                self.state = np.array([x,y, action_vel, action_phi, x_i,y_i, self.state[6], self.state[7], x_b,y_b,self.state[10],self.state[11]]) #keep velocity of invader and base station constant
        return Step(observation=self.state, reward=reward, done=done,info=info)

    def reset(self):
        self.state = np.array([20,0, 1.2, 0, 20,10, 1.0, float((math.pi)/4), 10,0,1.0,0]) #acc to constraint d3 <= d1
