...
class ChaserInvaderEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if((d3-d1)>0 and (d3-d1)<self.thresh_distance):
            ...
            info = ["still working"]
            self.state = np.array([x,y, action_vel, action_phi, x_i,y_i, self.state[6], self.state[7], 10,0,0,0.0]) #keep velocity of invader and base station constant
        return Step(observation=self.state, reward=reward, done=done,info=info)

    def reset(self):
        self.state = np.array([20,0, 1.2, 0, 10,10, 1.0, -float((math.pi)/2), 10,0,0.0,0.0]) #acc to constraint d3 <= d1
